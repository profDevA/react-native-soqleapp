...
export default class Slider extends PureComponent {
  ...
  render() {
    ...
    var minimumTrackStyle = {
      ...
      height: Animated.add(thumbLeft, thumbSize.height / 2),
      top: 0,
  ...
  _getThumbLeft = (value: number) => {
    ...
    return (
      ratio * (this.state.containerSize.height - this.state.thumbSize.height)
    );
  ...
  _getTouchOverflowSize = () => {
    ...
    if (state.allMeasured === true) {
      ...
      size.width = Math.max(
        0,
        props.thumbTouchSize.width - state.containerSize.width
      );
      size.height = Math.max(
        0,
        props.thumbTouchSize.height - state.thumbSize.height
      );
    }
